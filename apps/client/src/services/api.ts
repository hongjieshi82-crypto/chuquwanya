...
export async function tryRestoreSession() {
  ...
  try {
    ...
    if (isUnauthorizedApiError(reason)) {
      // Let the persistent auth provider refresh/expire its own session.
      // A backend 401 alone must not delete a valid browser refresh token.
      if (Platform.OS !== 'web') await clearAuthToken();
    }
    return null;
  }
}
