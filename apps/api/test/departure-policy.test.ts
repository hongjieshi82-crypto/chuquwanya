...
test('已核验的跨午夜窗口允许凌晨夜宵，闭园不足时长和未来不合适时段会被拒绝', () => {
  ...
  assert.ok(departureFailure(food, {departureMode:'now'}, new Date('2026-09-06T19:00:00Z')));
  const park = practicalActivities.find(a => a.id === 810002)!;
  assert.ok(departureFailure(park, {departureMode:'now'}, new Date('2026-09-06T12:00:00Z')));
...
test('跨午夜显示次日时间，不产生24点格式；标题关键词不能开放夜间权限', () => {
  ...
  assert.ok(departureFailure({...night, moodTags: []}, {departureMode: 'now'}, now));
  assert.match(departureFailure(night, {departureMode: 'now'}, new Date('2026-09-06T16:10:00Z'))!, /凌晨/);
});
...
test('现在出发不能在下午推荐环球完整一天或午餐路线', () => {
  const universal = beijingPracticalActivities.find((a) => a.id === 810001)!;
  const food = beijingPracticalActivities.find((a) => a.id === 810004)!;
  assert.match(departureFailure(universal, { departureMode: 'now' }, afternoon)!, /剩余时间/);
  assert.ok(departureFailure(food, { departureMode: 'now' }, afternoon));
});
...
test('晚上保留夜间短线，不再用 20:30 统一截止', () => {
  ...
  assert.match(departureFailure(daytime, { departureMode: 'now' }, evening)!, /剩余时间/);
...
test('夜间玩法不会推到过深夜', () => {
  ...
  assert.match(departureFailure(night, { departureMode: 'now' }, tooLate)!, /结束太晚/);
});
